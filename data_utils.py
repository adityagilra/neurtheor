...
def subplot_rasters(ax, listof_spiketimelists, runtime,\
    colorlist=['r','g','b'], labellist=['v1','v2','v3'], labels=True):
    for rnum,raster in enumerate(listof_spiketimelists):
        seglist = []
        for t in raster:
            ## append a segment for a spike
            seglist.append(((t,rnum+0.1),(t,rnum+0.9)))
        ## plot the raster
        if labels:
            segs = collections.LineCollection(seglist,\
                 color=colorlist[rnum],\
                 label=labellist[rnum])
        else:
            segs = collections.LineCollection(seglist,\
                 color=colorlist[rnum])
        ax.add_collection(segs)
    ax.set_xlim(0.0,runtime)
    rasterslen = len(listof_spiketimelists)
    if labels:
        ax.set_ylim(0,rasterslen+0.3) # extra 0.3 height for legend
        biglegend()
    else:
        ax.set_ylim(0,rasterslen)
...
def getSpikeIndices(Vm,Vt,takerisingedge=True):
    Vm = array(Vm)
    Vt_over_idxs = where(Vm>Vt)[0] # thresholded indices
    if takerisingedge:
        Vt_over_idxs = append( array([0]), Vt_over_idxs ) # prepend a zero to detect the first spike & get rising edges
        edges_idxs = where(diff(Vt_over_idxs)>2)[0] # only the rising edges indices within thresholded indices
        Vt_idxs = Vt_over_idxs[ edges_idxs+1 ] # rising edge indices, taking care of the 0 added before
    else:
        edges_idxs = where(diff(Vt_over_idxs)>2)[0] # only the falling edges indices within thresholded indices
        Vt_idxs = Vt_over_idxs[ edges_idxs ] # falling edge indices
    return Vt_idxs
